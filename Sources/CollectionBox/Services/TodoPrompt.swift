...
enum TodoPrompt {
    ...
    static func build(input: String, now: Date = Date(), lists: [String], lastList: String?) -> (system: String, user: String) {
        ...
        你是待办解析器。输出 JSON：
          ...
          规则：
          - title 去掉时间词、保留动作；例如「周五下午3点提醒我给妈妈打电话」→ title "给妈妈打电话"
          - due 输出带时区偏移的本地时间 ISO8601（如 2026-09-18T15:00:00+08:00），禁止裸 UTC；无法确定到期日时填 null 且 fallback=true
          - priority 仅当原文明示「重要/紧急」→ 1；「随便/不急」→ 9；否则 0
          - list 仅在原文点名某个列表时给（如「放到工作里」→ "工作"），否则 null
          - fallback=true 表示无法可靠解析（不限于时间），用于区分「无法解析」与「原文无时间词」
        """

...
/// WorkBuddy-style compact stats panel, parameterized by agent. Codex has no
...
struct AgentStatsView: View {
    ...
    private let service = AgentStatsService()
    ...
    private var metricContent: some View {
        if let stats = stats {
            ...
            Text("未找到 \(agent.label) 的本地会话数据").font(.system(size: 11)).foregroundStyle(.secondary).padding(.vertical, 10)
        }
    }
    ...
    private func refresh() {
        ...
        Task.detached(priority: .userInitiated) { [service] in
            let newStats = service.fetchStats(agent: agent, for: range)
            let newTrend = service.fetchTrend(agent: agent, for: range)
            await MainActor.run {
